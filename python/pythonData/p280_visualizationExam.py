...
# font_location='/.virtualenvs/pythonData/lib/site-packages/matplotlib/mpl-data/fonts/ttf/NanumBarunGothic.ttf'
...
filename = 'p280_visualizationExam01.png'
...
result = newgrouping.agg(myroot)
print(result)
print('-' * 50)

newDf = df.loc[:, ['평균','개수']]
newDf.plot(kind='bar', rot=0)
plt.title('3개의 극장의 평균과 상영관 수수')
filename = 'p280_visualizationExam02.png'
plt.savefig(filename, dpi=400, bbox_inches='tight')
print(filename + ' saved')
#plt.show()
...
filename = 'p280_visualizationExam03.png'
plt.savefig(filename, dpi=400, bbox_inches='tight')
print(filename + ' saved')
plt.show()